...
namespace OperaHouse_Assignment5
{
    public class Event : IComparable<Event>
    {
        ...
        public double SellTicket(Ticket ticket)
        {
            double ticketsforSale = 0;
            int numSold = 0;
            foreach (Ticket t in tickets)
            {
                if (t.SeatNumber == ticket.SeatNumber)
                {
                    ticketsforSale += t.Price;
                    numSold++;
                    t.Sold = true;
                }
            }
        ...
        public double ReturnTicket(Ticket ticket)
        {
            double result = 0;
            foreach (Ticket t in tickets)
            {
                if (t.SeatNumber == ticket.SeatNumber && t.Sold)
                {
                    result += t.Price;
                    t.Sold = false;
                }
